...
func NewRequest() *SuperAgent {
	...
	debug := os.Getenv("HTTP_REQUEST_DEBUG") == "1"
...
func (s *SuperAgent) SendFile(file interface{}, args ...interface{}) *SuperAgent {
	...
	default:
		...
		s.Errors = append(
			s.Errors,
			errors.New("SendFile currently only supports either a string (path/to/file), a slice of bytes (file content itself), or a os.File"),
		)
	}
...
func (s *SuperAgent) getResponseBytes() (Response, []byte, []error) {
	...
	if s.Debug {
		...
		if err != nil {
			...
			fmt.Printf("HTTP Request: %s", string(dump))
		}
	}
	...
	if s.Debug {
		...
		if nil != err {
			...
			fmt.Printf("HTTP Response: %s", string(dump))
		}
	}
